...
function jouer() {
  ...
  if (proposition < borneMin || proposition > borneMax) {
    ...
    if (proposition === nombreCache) {
      ...
      message = "Bravo ! Vous avez gagné<br>en " + nbreCoupsJoues + " essai(s) !!";
      partieTerminee = true;
    }
    else {
      ...
      if (nbreCoupsJoues < nbreCoupsMax) {
        ...
        message += " - vous avez perdu, le nombre était " + nombreCache;
        partieTerminee = true;
      }
    }
  }
...
function abandonner() {
  const message = "Pourquoi abandonner?<br>Le nombre caché était : " + nombreCache;
  document.getElementById("message").innerHTML = message;
  // cacher les boutons jouer et abandonner
  // montrer le bouton rejouer
  document.getElementById("partieEncours").style.display = 'none';
